...
fn part2(file: std.fs.File, allocator: Allocator) !u64 {
    ...
    while (try reader.readUntilDelimiterOrEofAlloc(allocator, '\n', stat.size)) |line| {
        ...
        outer: while (i < line.len - 7) : (i += 1) {
            ...
            if (std.mem.eql(u8, slice[0..4], "do()"))
                enabled = true;
            if (std.mem.eql(u8, slice[0..7], "don't()"))
                enabled = false;
...
test "part 2" {
    const file = std.fs.cwd().openFile("../../../inputs/2024/day03/test_2.txt", .{ .mode = .read_only }) catch return error.FileNotFound;
    defer file.close();
    const allocator = std.testing.allocator;
